...
B_BIP::B_BIP( TMdPrm *prm, uint16_t id, uint16_t n, bool has_params) : DA(prm), ID(id<<12), count_n(n), with_params(has_params)//, numReg(0)

{
	...
	for (int i = 1; i <= count_n; i++)	{
		...
		if (with_params){
		    ...
		    fld->setReserve(TSYS::strMess("%d:7",i));
			mPrm->p_el.fldAdd(fld = new TFld(TSYS::strMess("minF_%d",i).c_str(),TSYS::strMess(_("Minimum frequency %d"),i).c_str(),TFld::Real,TVal::DirWrite));
		    fld->setReserve(TSYS::strMess("%d:8",i));
			mPrm->p_el.fldAdd(fld = new TFld(TSYS::strMess("maxF_%d",i).c_str(),TSYS::strMess(_("Maximum frequency %d"),i).c_str(),TFld::Real,TVal::DirWrite));
		    fld->setReserve(TSYS::strMess("%d:8",i));
