...
uint16_t B_BTE::HandleEvent(int64_t tm, uint8_t * D)
{
	...
	if (ft3ID.k && (ft3ID.k <= count_n)) {
	    ...
	    case 4:
		if (with_params)
		    mPrm.vlAt(TSYS::strMess("alarms_%d", ft3ID.k)).at().setB(D[3], tm, true);
		l = 4;
		break;
	    case 5:
		if (with_params) {
	    ...
	    case 7:
		if (with_params)
		    mPrm.vlAt(TSYS::strMess("Ki_%d", ft3ID.k)).at().setR(TSYS::getUnalignFloat(D + 3), tm, true);
		l = 7;
		break;
	    case 8:
		if (with_params)
		    mPrm.vlAt(TSYS::strMess("Ku_%d", ft3ID.k)).at().setR(TSYS::getUnalignFloat(D + 3), tm, true);
		l = 7;
		break;
	    case 9:
		if (with_params)
		    mPrm.vlAt(TSYS::strMess("A_%d", ft3ID.k)).at().setR(TSYS::getUnalignFloat(D + 3), tm, true);
		l = 7;
		break;
	    }
	}
