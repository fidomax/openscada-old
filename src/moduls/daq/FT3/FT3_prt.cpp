...
void TProt::load_()
{
	...
	if(!SYS->selDB().empty()) {
		...
		if(itReg.find(db_ls[i_it]) == itReg.end() && SYS->chkSelDB(nAt(db_ls[i_it]).at().DB())) nDel(db_ls[i_it]);
	}
    } catch (TError &err) {
...
void TProt::modStart()
{
	...
	if(nAt(ls[i_n]).at().toEnable()) nAt(ls[i_n]).at().setEnable(true);
}
...
uint8_t TProt::CRCHi[] = {  };

uint8_t TProt::CRCLo[] = { ,
...
//		if(CRC16(rez.substr(0,rez.size()-2)) != (uint16_t)((rez[rez.size()-2]<<8)+(uint8_t)rez[rez.size()-1]))
	    ...
	    err = TSYS::strMess(_("Protocol '%s' error."), prt.c_str());

	//> Check respond pdu
	if(err.empty()) {
	    if(pdu.size() < 2) err = _("13:Error respond");
	    if(pdu[0] & 0x80) switch(pdu[1]) {
	    case 0x1:
		err = TSYS::strMess(_("1:%02X:Function is not supported."), (unsigned char) (pdu[0] & (~0x80)));
		break;
	    case 0x2:
		err = _("2:Requested address not allow or request area too long.");
		break;
	    case 0x3:
		err = _("3:Illegal data value into request.");
		break;
	    case 0x4:
		err = _("4:Server failure.");
		break;
	    case 0x5:
		err = _("5:Request requires too long time for execute.");
		break;
	    case 0x6:
		err = _("6:Server is busy.");
		break;
	    case 0x7:
		err = _("7:Program function is error. By request functions 13 or 14.");
		break;
	    case 0xA:
	    case 0xB:
		err = _("10:Gateway problem.");
		break;
	    default:
		err = TSYS::strMess(_("12:%02X:Unknown error."), (unsigned char) (pdu[1]));
		break;
	    }
	}
    } catch (TError &er) {
	...
	if(!err.empty())
	    ...
	    mess += _("RESP -> ") + ((prt != "ASCII") ? TSYS::strDecode(rez, TSYS::Bin) : rez.substr(0, rez.size() - 2));
	pushPrtMess(mess + "\n");
    }
}
...
void TProt::cntrCmdProc(XMLNode *opt)
{
	...
	if(ctrMkNode("area", opt, 0, "/node", _("Nodes")))
	    ctrMkNode("list", opt, -1, "/node/node", _("Nodes"), RWRWR_, "root", SPRT_ID, 5, "tp", "br", "idm", "1", "s_com", "add,del", "br_pref", "n_",
		    "idSz", "20");
...
Node::Node(const string &iid, const string &idb, TElem *el) :
	TConfig(el), data(NULL), mId(cfg("ID")), mName(cfg("NAME")), mDscr(cfg("DESCR")), mPer(cfg("DT_PER").getRd()), mAEn(cfg("EN").getBd()), mEn(false),
	mDB(idb), prcSt(false), endrunRun(false), cntReq(0), FCB2(0), FCB3(0)
{
    mId = iid;
...
string Node::inTransport()
{
    return cfg("InTR").getS();
}
...
bool Node::cfgChange(TCfg &ce)
{
	...
	cfg("DT_PER").setView(false);
	cfg("DT_PROG").setView(false);
	cfg("TO_TR").setView(false);
	cfg("TO_PRT").setView(false);
	cfg("TO_ADDR").setView(false);

	//> Show selected
	switch(ce.getI()) {
	case 0:
	    cfg("ADDR").setView(true);
	    cfg("DT_PER").setView(true);
	    cfg("DT_PROG").setView(true);
	    break;
	case 1:
	    cfg("ADDR").setView(true);
	    cfg("TO_TR").setView(true);
	    cfg("TO_PRT").setView(true);
	    cfg("TO_ADDR").setView(true);
	    break;
	case 2:
	    cfg("TO_TR").setView(true);
	    cfg("TO_PRT").setView(true);
...
void Node::load_()
{
	...
	if(!SYS->selDB().empty()) {
	    nListBUC(db_ls);
	    for(unsigned i_it = 0; i_it < db_ls.size(); i_it++)
		if(itReg.find(db_ls[i_it]) == itReg.end() && SYS->chkSelDB(nAtBUC(db_ls[i_it]).at().DB())) nDelBUC(db_ls[i_it]);
	}
    } catch (TError &err) {
	mess_err(err.cat.c_str(), "%s", err.mess.c_str());
...
//				GetBE(&msg->C);
	...
	case AddrReq:
	    ...
	    ans.L = 6;
	    ans.C = 8; // положительная квитанция
	    ans.D[0] = 0;
	    ans.D[1] = 0;
	    ans.D[2] = 0;
	    ans.D[3] = 0;
...
//	ctrMkNode("oscada_cntr",opt,-1,"/",_("Node: ")+name(),RWRWR_,"root",SPRT_ID);
	...
	if(ctrMkNode("area", opt, -1, "/nd", _("Node"))) {
		...
		ctrMkNode("fld", opt, -1, "/nd/st/db", _("DB"), RWRWR_, "root", SPRT_ID, 4, "tp", "str", "dest", "select", "select", "/db/list", "help",
			TMess::labDB());
	    }
	...
	if(ctrChkNode(opt, "add", RWRWR_, "root", SPRT_ID, SEC_WR)) {
	    ...
	    string vid = TSYS::strEncode(opt->attr("id"), TSYS::oscdID);
	    nAddBUC(vid);
	    nAtBUC(vid).at().setName(opt->text());
	}
	...
	for(unsigned i_s = 0; i_s < sls.size(); i_s++)
	    opt->childAdd("el")->setText(sls[i_s]);
    } else if(a_path.substr(0, 7) == "/nd/cfg")
	...
	if(ctrChkNode(opt, "SnthHgl", RWRWR_, "root", SDAQ_ID, SEC_RD)) try {
	    SYS->daq().at().at(TSYS::strParse(progLang(), 0, ".")).at().compileFuncSynthHighl(TSYS::strParse(progLang(), 1, "."), *opt);
	} catch (...) {
	}
	...
	case 0:
		...
		if(!SYS->daq().at().at(ls[i_l]).at().compileFuncLangs())
		    ls.erase(ls.begin() + i_l);
		else
		    i_l++;
	    break;
	case 1:
	    if(SYS->daq().at().modPresent(TSYS::strSepParse(tplng, 0, '.'))) SYS->daq().at().at(TSYS::strSepParse(tplng, 0, '.')).at().compileFuncLangs(&ls);
	    break;
	}
...
NodeBlock::NodeBlock(const string &iid, const string &idb, TElem *el) :
	TFunction("FT3Block_" + iid), TConfig(el), mId(cfg("ID")), mName(cfg("NAME")), mDscr(cfg("DESCR")), mAEn(cfg("EN").getBd()), mEn(false), mDB(idb),
	prcSt(false), endrunRun(false), cntReq(0)
{
    mId = iid;
    //    mNode = grpAdd("n_");
    //cfg("MODE").setI(0);
...
string NodeBlock::tbl()
{
    return owner().id() + "_block";
}
...
//    	    	if(io(i_io)->flg()&IsLink) {
		ctrMkNode("fld", opt, -1, ("/lnk/el_" + TSYS::int2str(i_io)).c_str(), io(i_io)->name(), enableStat() ? R_R_R_ : RWRWR_, "root", SPRT_ID, 3,
			"tp", "str", "dest", "sel_ed", "select", ("/lnk/ls_" + TSYS::int2str(i_io)).c_str());
//    	    	}
	    }
	}
	return;
...
//    			if(ctrChkNode(opt,"set",RWRWR_,"root",SPRT_ID,SEC_WR))	setDB(opt->text());
		...
		if(a_path.substr(0, 7) == "/nd/cfg") {
			...
			case 0:
			    ...
			    break;
			case 1:
			    if(SYS->daq().at().modPresent(prm0)) SYS->daq().at().at(prm0).at().list(ls);
			    break;
			case 2:
			...
			if(a_path.substr(0, 8) == "/lnk/el_") {
				...
				io(atoi(a_path.substr(8).c_str()))->setRez(opt->text());
				modif();
			    }
			}
		    }
