...
//		if(mess_lev() == TMess::Debug) mess_sys(TMess::Debug, _("%d empty %08X %08X"), msg->B, pC, pC->head);
	...
	} else if (mess_lev() == TMess::Debug) mess_sys(TMess::Debug, _("ReqData BAD FCB!!!"));
...
//		if(mess_lev() == TMess::Debug) mess_sys(TMess::Debug, _("OOOOOOOO"));
	...
	} else if (mess_lev() == TMess::Debug) mess_sys(TMess::Debug, _("SetData BAD FCB!!!"));
...
//> Get page info
	...
	//Hide cfg
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BUC");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BVTS");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BVT");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BVI");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BIP");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_PAUK");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BTU");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_ACCOUNT");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BTR");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_BTE");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_ODOR");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_GZD");
	ctrRemoveNode(opt, "/cntr/cfg/PRM_BD_GNS");
...
//mess_sys(TMess::Error, _("GetState"));
    if (mDA) rc = mDA->GetState();
    if (rc == BlckStateUnknown) mess_sys(TMess::Error, _("Receiving block state error"));
    return rc;
}
...
uint16_t TMdPrm::BlckSetupClock(void)
{
    ...
    if (mDA) rc = mDA->SetupClock();
    if ((rc == BAD2) || (rc == BAD3)) mess_sys(TMess::Error, _("Can't SetupClock"));
    else if (rc == ERROR) mess_sys(TMess::Error, _("No answer to SetupClock"));
    return rc;
}
...
uint16_t TMdPrm::BlckPreInit(void)
{
    ...
    if (mDA) rc = mDA->PreInit();
    if ((rc == BAD2) || (rc == BAD3)) mess_sys(TMess::Error, _("Can't PreInit"));
    else if (rc == ERROR) mess_sys(TMess::Error, _("No answer to PreInit"));
    if (rc == GOOD2) {
	vector<string> lst;
	list(lst);
	for (int i_l = 0; i_l < lst.size(); i_l++) {
...
uint16_t TMdPrm::BlckSetParams(void)
{
    ...
    if (mDA) rc = mDA->SetParams();
    if ((rc == BAD2) || (rc == BAD3)) mess_sys(TMess::Error, _("Can't SetParams"));
    else if (rc == ERROR) mess_sys(TMess::Error, _("No answer to SetParams"));
    if (rc == GOOD2) {
	vector<string> lst;
	list(lst);
	for (int i_l = 0; i_l < lst.size(); i_l++) {
	    AutoHD<TMdPrm> t = at(lst[i_l]);
	    rc = t.at().BlckSetParams();
	    if (rc != GOOD2)
		break;
	}
    }
...
uint16_t TMdPrm::BlckPostInit(void)
{
    ...
    if (mDA) rc = mDA->PostInit();
    if ((rc == BAD2) || (rc == BAD3)) mess_sys(TMess::Error, _("Can't PostInit"));
    else if (rc == ERROR) mess_sys(TMess::Error, _("No answer to PostInit"));
    if (rc == GOOD2) {
	vector<string> lst;
	list(lst);
	for (int i_l = 0; i_l < lst.size(); i_l++) {
	    AutoHD<TMdPrm> t = at(lst[i_l]);
	    rc = t.at().BlckPostInit();
	    if (rc != GOOD2)
		break;
	}
    }
...
uint16_t TMdPrm::HandleEvent(time_t tm, uint8_t * D)
{
    ...
    for (int i_l = 0; i_l < lst.size(); i_l++) {
	AutoHD<TMdPrm> t = at(lst[i_l]);
	rc = t.at().HandleEvent(tm, D);
	if (rc)
	    break;
    }

    if (!rc && mDA) rc = mDA->HandleEvent(tm, D);
    return rc;
}
...
uint8_t TMdPrm::cmdGet(uint16_t prmID, uint8_t * out)
{
    ...
    if (!rc && mDA) rc = mDA->cmdGet(prmID, out);
    return rc;
}
...
uint8_t TMdPrm::cmdSet(uint8_t * req, uint8_t addr)
{
    ...
    if (!rc && mDA) rc = mDA->cmdSet(req, addr);
    return rc;
}
