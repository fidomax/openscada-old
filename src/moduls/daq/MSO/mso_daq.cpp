...
TMdContr::TMdContr(string name_c, const string &daq_db, TElem *cfgelem) :
		TController(name_c, daq_db, cfgelem), prc_st(false), endrun_req(false), tmGath(0), numRx(0), numTx(0), mSched(cfg("SCHEDULE")), mPrior(
				cfg("PRIOR").getId()), mAddr(cfg("ADDR")), mNode(cfg("NODE").getId()), reqTm(cfg("TM_REQ").getId())
{
...
string TMdContr::getStatus()
{
	...
	if (startStat() && !redntUse()) {
		if (!prc_st)
			val += TSYS::strMess(_("Task terminated! "));
		if (period())
...
bool TTpContr::DataIn(const string &ireqst, const uint32_t node)
{
	...
	if (mess_lev() == TMess::Debug)
		mess_debug("DataIn", " sender<%u> mso<%u> channel<%u> type<%u> param<%u> flag<%u>", node, mso, channel, type, param, flag);
	vector<string> lst;
...
void TMdContr::prmEn(TMdPrm *prm, bool val)
{
	...
	if (val && i_prm >= pHd.size())
		pHd.push_back(prm);
	if (!val && i_prm < pHd.size())
		pHd.erase(pHd.begin() + i_prm);
}
...
void TMdPrm::enable()
{
	if (enableStat())
		return;
	if ((type().name == "tp_MSO") && (mDA == NULL))
		mDA = new MSOparam(this, cfg("DEV_ID").getI());
	if ((type().name == "tp_TT") && (mDA == NULL))
		mDA = new MezTT(this, cfg("DEV_ID").getI());
	if ((type().name == "tp_TC") && (mDA == NULL))
		mDA = new MezTC(this, cfg("DEV_ID").getI());
	if ((type().name == "tp_TU") && (mDA == NULL))
		mDA = new MezTU(this, cfg("DEV_ID").getI());
	if ((type().name == "tp_TI") && (mDA == NULL))
		mDA = new MezTI(this, cfg("DEV_ID").getI());
	if ((type().name == "tp_TR") && (mDA == NULL))
		mDA = new MezTR(this, cfg("DEV_ID").getI());
	...
	if (!enableStat() || !owner().startStat()) {
		if (val.name() == "err") {
			if (!enableStat())
				...
				val.setS(_("2:Acquisition is stoped."), 0, true);
		} else
...
{
	if (!enableStat() || !owner().startStat())
		valo.setI( EVAL_INT, 0, true);
	string rez;

//> Send to active reserve station
	if (owner().redntUse()) {
		if (valo.getS(0, true) == pvl.getS())
			return;
		XMLNode req("set");
